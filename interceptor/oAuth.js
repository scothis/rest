...
function defaultOAuthCallback(hash) {
	const queryString = hash.indexOf('#') === 0 ? hash.substring(1) : hash;
	const params = {};
	const regex = /([^&=]+)=([^&]*)/g;
...
function defaultWindowStrategy(url) {
	const w = window.open(url, '_blank', 'width=500,height=400');
	return close.bind(w);
}
...
function authorize(config) {
	return when.promise((resolve) => {
		...
		const state = Math.random() * new Date().getTime();
		const url = new UrlBuilder(config.authorizationUrlBase).build({
			'response_type': 'token',
		...
		const dismissWindow = config.windowStrategy(url);

		pubsub.subscribe(state, (authorization) => {
			dismissWindow();
...
export default interceptor({
	...
	request(request, config) {
		...
		if (config.token) {
			...
			return authorize(config).then((authorization) => {
				request.headers.Authorization = config.token = authorization;
				return request;
			});
		}
	},
	response(response, config, client) {
